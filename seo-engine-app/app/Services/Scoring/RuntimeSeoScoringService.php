...
class RuntimeSeoScoringService extends SeoScoringService
{
    ...
    public function audit(object $page, array $searchConsoleData = []): array
    {
        ...
        $isPublished = in_array((string) ($page->status ?? ''), ['published'], true) || ! empty($page->published_at);
        $indexed = null;

        if (property_exists($page, 'indexed') && $page->indexed !== null) {
            $indexed = $page->indexed;
        } elseif (property_exists($page, 'is_indexed') && $page->is_indexed !== null) {
            $indexed = $page->is_indexed;
        } elseif (array_key_exists('indexed', $searchConsoleData) && $searchConsoleData['indexed'] !== null) {
            $indexed = $searchConsoleData['indexed'];
        }

        if ($indexed === false || ($isPublished && $indexed !== true)) {
            $score -= 10;
            $issues[] = 'not_indexed';
