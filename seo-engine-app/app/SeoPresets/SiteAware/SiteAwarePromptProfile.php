...
final class SiteAwarePromptProfile implements PromptProfileProvider
{
    ...
    public function generationCorePrompt(string $keyword, string $cluster, array $blueprint, array $editorialSections, array $expectedSignals): string
    {
        ...
        return $langLine."\n"
            ...
            .'Niche détectée : '.($blueprint['niche'] ?? 'generic')."\n"
            .'Voix éditoriale attendue : '.($blueprint['voice_note'] ?? '')."\n"
            ...
            .'Profondeur métier à couvrir (H2/H3 dédiés si pertinent) : '.json_encode($blueprint['depth_topics'] ?? [], JSON_UNESCAPED_UNICODE)."\n"
            .'Scénarios contextualisés (troisième personne, pas de récit « je ») : '.json_encode($blueprint['field_scenarios'] ?? [], JSON_UNESCAPED_UNICODE)."\n"
            .'Arbitrages client à expliciter : '.json_encode($blueprint['arbitrages'] ?? [], JSON_UNESCAPED_UNICODE)."\n"
            ."Forme attendue :\n"
            ."- un seul article continu, voix éditoriale institutionnelle de {$siteName} — interdit je/j'/mon/ma/mes\n"
            ."- H2/H3 avec titres métier concrets (jamais numérotés, jamais \"Zoom terrain\", jamais \"Guide\")\n"
            ."- profondeur d'abord (cadre, acteurs, documents, cas d'usage) ; listes courtes seulement après un paragraphe de contexte\n"
            ...
            .'- '.FieldExpertWritingDirectives::minWordCount().' mots minimum dans content (cible 1600–2000 pour une requête informationnelle forte)'."\n"
    ...
    public function generationFaqPrompt(string $keyword, string $cluster, array $blueprint, string $title, string $metaDescription, string $h1, string $content): string
    {
        ...
        return ($language === 'fr'
            ...
            .'H1 : '.$h1."\n"
            ."Contraintes FAQ :\n"
            ."- 4 à 6 questions métier obligatoires (responsabilités, documents, délais, copropriété, erreurs fréquentes…)\n"
            ."- questions formulées comme un décideur métier ou un responsable technique\n"
