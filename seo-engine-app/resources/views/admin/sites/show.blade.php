        ...
        <form method="POST" action="{{ route('admin.sites.google-connection.update', $site->site_id) }}" class="px-6 py-5 space-y-4">
            ...
            <div class="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                ...
                @if(!empty($gscSyncDetails))
                    ...
                    @endif
                    @if(!empty($gscSyncDetails['analytics']) && is_array($gscSyncDetails['analytics']))
                        <div class="mt-3 space-y-2">
                            @foreach($gscSyncDetails['analytics'] as $label => $diag)
                                <div class="rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs">
                                    ...
                                    <div class="mt-1 text-gray-500">
                                        statut {{ $diag['status'] ?? '—' }} · HTTP {{ $diag['http_code'] ?? '—' }} · rows {{ $diag['row_count'] ?? 0 }}
                                    </div>
                                    @if(!empty($diag['reason']))
                                        <div class="mt-1 text-amber-600">raison : {{ $diag['reason'] }}</div>
                                    @endif
                                </div>
                            @endforeach
                        </div>
